	...
	<div>
		<div class="p-2 rounded-md bg-primary-800 gap-2 flex flex-col">
			...
			<Checkbox
				...
				:modelValue="store.settings.value?.setsGrayUnownedGrid"
				@change="store.toggle.setsGrayUnownedGrid"
			/>
			<Checkbox
				label="Gray Out Unowned Cards (List)"
				:modelValue="store.settings.value?.setsGrayUnownedList"
				@change="store.toggle.setsGrayUnownedList"
			/>
			<Checkbox
				label="Always Show Owned Heart (List)"
				:modelValue="store.settings.value?.setsShowOwnedHeartList"
				@change="store.toggle.setsShowOwnedHeartList"
			/>
			<Checkbox
				label="Display as List"
				:modelValue="store.settings.value?.setsDisplayAsList"
				@change="store.toggle.setsDisplayAsList"
			/>

			<div class="grid grid-cols-[3fr_10fr] items-center">
				<span>Card Size</span>
				<select
					:value="store.settings.value?.setsGridCardSize"
					@change="(e) => store.set.setsGridSize((e.target as HTMLSelectElement).value as keyof typeof EListSize)"
			...
			<div class="grid grid-cols-[3fr_10fr] items-center">
				<span>List Size</span>
				<select
					:value="store.settings.value?.setsListSize"
					@change="(e) => store.set.setsListSize((e.target as HTMLSelectElement).value as keyof typeof EListSize)"
					class="bg-primary-700 border border-primary-600 rounded-md px-2 py-1 focus:outline-none focus:border-accent-500"
				>
					<option v-for="(label, key) in EListSize" :key="key" :value="key">
						{{ label }}
