...
interface IProps {
	...
	archetypeInfo: {name: string; cardCount: number}
}
...
const styles = computed(() => getCardStyles(props.card))
</script>
	...
	>
		...
		>
			...
			<div class="flex gap-4 items-center">
				<span class="text-lg font-bold">{{ props.archetypeInfo.name }}</span>
				<span class="font-semibold text-contrast-600"
					>{{ props.archetypeInfo.cardCount }} Cards</span
				>
			</div>
		</div>
	</div>
