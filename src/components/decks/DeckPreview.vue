...
import DeckContextMenu from './DeckContextMenu.vue'
...
const emit = defineEmits<{
	...
	(e: 'delete'): void
	(e: 'clone'): void
	(e: 'move', targetFolder: string): void
}>()
...
<template>
	<DeckContextMenu
		...
		@rename-deck="() => emit('rename')"
		@delete-deck="() => emit('delete')"
		@clone-deck="() => emit('clone')"
		@move-deck="(targetFolder) => emit('move', targetFolder)"
	>
		<div
		...
		>
				...
				<div class="bg-accent-500 w-43.25 h-64.5 rounded-sm center-shell relative">
					...
					>
						...
						>
							...
							>
								<Icon icon="material-symbols:swap-horiz-rounded" class="text-sm" />
			...
			<div
				...
				style="
					box-shadow:
						inset 0 0 30px rgba(var(--color-accent-400-rgb), 0.2),
