...
export async function loadOtherImage(
	imageUrl: string,
	path: string,
	imgName: string
): Promise<ImageResult> {
	try {
		const appPath = await Path.AppRoot()
		const localPath = `${path.startsWith('/') ? '' : '/'}${path}/${imgName}.jpg`
		const fullLocalPath = appPath + localPath
		...
		const downloadResult = await window.electronImage.downloadImage(imageUrl, fullLocalPath)
		if (!downloadResult.success) {
			throw new Error(`Failed to download non-card image: ${downloadResult.error}`)
		}

		return {success: true, localPath: fullLocalPath}
	} catch (error) {
		const errorMessage = error instanceof Error ? error.message : String(error)
		console.error(`Error loading non-card image ${imgName}:`, errorMessage)
		return {success: false, error: errorMessage}
	}
}
