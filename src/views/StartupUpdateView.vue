...
<template>
	<div class="w-full min-h-full">
		<div
			class="bg-black/80 absolute inset-0 flex items-center justify-center"
		>
			<div>
				...
				<div v-else-if="state === 'needs-setup'">
					<div class="font-semibold">
						<h3 class="text-lg font-bold">
							Welcome to YGO Tallymaster!
						</h3>
						<p>
							Before we can get started, we need to set up your
							database.
						</p>
						<p>
				...
				<div v-else-if="state === 'update-available'">
					<div class="font-semibold">
						...
						<div
							class="w-full flex items-center justify-center gap-4 pt-4"
						>
							<button
								@click="startSetup"
								class="bg-accent-500 hover:bg-accent-400 p-2 rounded-md cursor-pointer transition-colors"
					...
					<div class="w-64 bg-gray-700 rounded-full h-2 mx-auto">
						<div
							...
							:style="{
								width:
									totalSteps > 0
										? (updateStep / totalSteps) * 100 + '%'
										: '0%',
							}"
						></div>
					</div>
				</div>
				<div v-else-if="state === 'ready'" class="text-xl font-bold">
					You are ready to go!
				</div>
